# Manifest Arsip Lengkap — SINABOS 4.2.1
...
| `sql/` | `000-preflight.sql`, `001-v4.sql` (instalasi penuh), `002-verify.sql`, `003-maintenance.sql`, `004-v4.1.sql` (ISBN/Edisi + resetBooks), `005-tema-akun.sql` (tema per akun 4.2.0 — jalankan SEBELUM deploy 4.2.0), `006-tampilan-login.sql` (tema halaman login 4.2.1 — tanpa perubahan tabel, aman diulang), `000-diagnostik-v4.sql` | Panduan perubahan: `docs/PANDUAN-PERUBAHAN-DATABASE.md` |
...
| `tests/` | 53 uji unit/DB/migrasi/CSV (node:test + PGlite), `concurrency.pg.mjs` (PostgreSQL asli + benchmark), fixture schema v3, uji browser Playwright (13) | |
...
| `artifacts/` | **Bukti pengujian**: hasil 53 uji unit (TAP), PostgreSQL native + benchmark JSON, 13 uji browser, kompilasi Cloudflare, audit dependensi, tangkapan layar antarmuka, ukuran bundle | Data di dalamnya simulasi |
