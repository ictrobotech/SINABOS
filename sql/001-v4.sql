...
CREATE TABLE IF NOT EXISTS public.books (
 ...
 kurikulum text, sumber_buku text, total_buku integer NOT NULL DEFAULT 0 CHECK(total_buku>=0),
...
DECLARE
 ...
 month_start date; month_end date; code text; title text; subject text; class_name text; src text;
BEGIN
 IF p_action='me' THEN
 ...
 IF p_action IN ('dashboard','books','saveBook','stockIn','stockLoss','rombel','saveRombel','users','saveUser','resetPassword','report','audit','active','health') AND u.role<>'admin' THEN
 ...
 ELSIF p_action='saveBook' THEN
  ...
  IF entity IS NULL THEN
   ...
   INSERT INTO public.books(kode_buku,judul,mata_pelajaran,kelas_target,tahun_terbit,penulis,penerbit,kurikulum,sumber_buku)
   VALUES(code,title,subject,class_name,p->>'tahun_terbit',trim(p->>'penulis'),trim(p->>'penerbit'),trim(p->>'kurikulum'),src) RETURNING id INTO entity;
  ...
  ELSE
   ...
   UPDATE public.books SET kode_buku=code,judul=title,mata_pelajaran=subject,kelas_target=class_name,tahun_terbit=p->>'tahun_terbit',
    penulis=trim(p->>'penulis'),penerbit=trim(p->>'penerbit'),kurikulum=trim(p->>'kurikulum'),sumber_buku=src,aktif=coalesce((p->>'aktif')::boolean,true),revision=revision+1 WHERE id=entity;
  END IF;
...
DECLARE u public.sinabos_users%rowtype; p jsonb:=coalesce(p_payload,'{}'); result jsonb; previous public.sinabos_requests%rowtype;
 rid uuid; fingerprint text; is_mutation boolean:=p_action IN ('circulate','saveBook','stockIn','stockLoss','saveRombel','saveUser','resetPassword');
