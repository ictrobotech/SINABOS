-- SINABOS 4.2.0 — tema tersimpan per akun (modern/profesional/emerald); hanya admin boleh mengubah.
-- Idempotent — aman dijalankan ulang. File kecil: boleh psql atau SQL Editor web (jalankan utuh sekali jalan).
-- Jalankan SEBELUM deploy aplikasi 4.2 (frontend menunggu aksi saveTheme).
BEGIN;
SELECT pg_advisory_xact_lock(734120260001::bigint);
ALTER TABLE public.sinabos_users ADD COLUMN IF NOT EXISTS tema text NOT NULL DEFAULT 'modern'
 CHECK(tema IN ('modern','profesional','emerald'));
