...
create table if not exists books (
  ...
  kurikulum text,
  sumber_buku text,
  total_buku integer not null default 0 check (total_buku >= 0),
...
alter table books add column if not exists tahun_terbit text;
alter table books add column if not exists penulis text;
alter table books add column if not exists penerbit text;
alter table books add column if not exists kurikulum text;
alter table books add column if not exists sumber_buku text;
alter table books add column if not exists total_buku integer default 0;
alter table books add column if not exists sumber_dana text default 'BOS';
update books set total_buku = 0 where total_buku is null;
...
group by b.id, b.kode_buku, b.judul, b.mata_pelajaran,
         b.kelas_target, b.total_buku, b.sumber_dana, b.aktif;
...
select
  ...
  (array_agg(t.nama_guru order by t.tanggal desc, t.created_at desc))[1] as peminjam_akhir,
...
select
  ...
  string_agg(initcap(ci.kondisi), '; ' order by b.judul) as kondisi
from circulation_transactions t
join circulation_items ci on ci.transaction_id = t.id
join books b on b.id = ci.buku_id
